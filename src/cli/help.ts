...
function renderHelpBanner(version: string, colors: HelpColors): string {
  const subtitle =
    "Prompt + files required — GPT-5.5 Pro/GPT-5.4 Pro/GPT-5.4 for tough questions with code/file context.";
...
function renderHelpFooter(program: Command, colors: HelpColors): string {
  const tips = [
    ...
    `${colors.bullet("•")} Non-preview runs spawn detached sessions (especially Pro API models such as gpt-5.5-pro/gpt-5.4-pro). If the CLI times out, do not re-run — reattach with ${colors.accent("oracle session <slug>")} to resume/inspect the existing run.`,
